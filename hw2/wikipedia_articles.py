...
headers = {'User-Agent': 'EducationalScraper/67 )'}
...
def search(start, max_depth = 3):
    ...
    while not len(q) == 0:
        ...
        if cur_depth < max_depth:
            ...
            if len(cur_links) > int(args.max_links_per_page):
                cur_links = random.sample(cur_links, int(args.max_links_per_page))
            out[page.split("/")[-1]] = [kega.split("/")[-1] for kega in cur_links]
